    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                navigationIcon = {
                    IconButton(onClick = {
                        navController.popBackStack() //para no crear multiples hojas
                    }) {
                        Icon(
        ...
        Column(
            modifier = Modifier
                ...
                .padding(horizontal = 32.dp).imePadding().verticalScroll(
                    rememberScrollState()
                ),
            ...
            Button(
                onClick = {
                    ...
                    val isvalidcontraseña = validatePassword(inputContraseña).first
                    val isvalidconfirmacion = validatePasswordConfirm(inputContraseña, inputPasswordConfirmation).first

                    nameError = validateName(inputName).second
                    emailError = validateEmail(inputEmail).second
                    passwordError = validatePassword(inputContraseña).second
                    passwordconfirmError = validatePasswordConfirm(inputContraseña,inputPasswordConfirmation).second

                    if (isvalidname && isvalidemail && isvalidcontraseña &&isvalidconfirmacion) {
                        auth.createUserWithEmailAndPassword(inputEmail, inputContraseña).addOnCompleteListener(activity) { task ->
                            if (task.isSuccessful) {
                                ...
                                registerError = when (task.exception) {
                                    ...
                                    else -> "Error al registrarse. Intente de nuevo"
                                }
                            }
                        }
                    }else{
                        registerError = "Hubo un error en el registro"
                        print("Error linea 251 en register")
                    }
